...
from function.data_google_sheets import leer_Datos_google_sheets
import pandas as pd
import sqlite3
...
if not st.session_state["autenticado"]:
        ...
        if boton_login:
            if user_input and pass_input:
                username_db, clave_db = leer_Datos_google_sheets()
                login(user_input, pass_input, username_db, clave_db)
            else:
    ...
    st.sidebar.divider()
    button_cerrar_sesion = st.sidebar.button("Cerrar Sesión")
    if button_cerrar_sesion:
        st.session_state["autenticado"] = False
        st.rerun()
    ...
    if herramientas == "Nuevo Proyecto":
        with st.form("Carga de nuevo proyecto: "):
            ...
            url = st.file_uploader("Imagen o Video",["png","mp4"])
